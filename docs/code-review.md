...
- [x] **Password hash exposed in GraphQL.** `back-end/src/user/user.schema.ts` puts `@Field()` on `password`, so `schema.gql` exposes `User.password: String!` — any client can query the bcrypt hash. Remove the decorator (see also the DTO-split item below).
- [x] **Stale cookie breaks the whole site.** The GraphQL context factory inlined in `back-end/src/app.module.ts` throws `UnauthorizedException` when a present token fails to verify, so an expired `jwt` cookie makes even public queries (`getActivities`, `getCities`) fail. Only reject in the guard, for protected operations. Fixed 2026-09-22 in two steps: first the context factory treated an unverifiable token as anonymous (`jwtPayload: null` — the throw actually surfaced as HTTP 500, not 401), then the auth consolidation below removed the context-factory token handling entirely. Pinned by e2e tests (`app.e2e.spec.ts`: expired and garbage cookies + public queries succeed, protected operations still rejected).
...
- [x] **One auth module of record.** Auth is spread across ~15 files with three token transports (httpOnly cookie, localStorage `token`, `jwt` header) that are never reconciled. **Decided: the cookie is the single transport; the client derives "logged in" from `getMe` (delete, don't build).** Done 2026-09-22; e2e tests pin the contract (cookie works, `jwt` header rejected, stale cookie only fails guarded operations).
  - [x] Back-end: move token extract/verify out of the `app.module.ts` context factory into the auth module, behind the guard (also fixes the stale-cookie site-wide failure above). `cookie-parser` is now registered by the root `BaseAppModule` (not `main.ts` bootstrap), so test apps built from the module tree parse cookies too — previously the e2e app silently had no cookie support, which is why its `getMe` test used the header transport.
  - [x] Back-end: `login` returns the `User`, not `access_token` — nothing consumes a token in the body anymore. `SignInDto` deleted (also removes the schema's only snake_case field, noted under Naming).
  - [x] Back-end: drop the write-only `user.token` column and `updateToken` (written, never read).
  - [x] Back-end: shorten `JWT_EXPIRATION_TIME` — it's ~31 years today, and with no revocation the expiry is the only bound on a stolen token. Set `SameSite=Lax` (+ `secure` in prod) on the cookie in `auth.resolver.ts`. Chose 604800s (7 days); `logout` clears with the same options so the clear matches.
  - [x] Front-end: `AuthProvider` runs `getMe` on mount; `user`-or-`null` is the whole auth state. Delete the localStorage token logic in `contexts/authContext.tsx` (a stored JWT is an XSS-readable credential). Sign-in sets `user` straight from the `login` payload (same fields as `getMe`) instead of a second round-trip.
...
- [ ] **Single config seam per side.** Back-end mixes `ConfigService` with raw `process.env` (`MONGO_URI` in `app.module.ts`, `FRONTEND_URL` in `main.ts`; `auth.resolver.ts` now goes through `ConfigService`). Front-end hard-codes URLs in `graphql/apollo.ts`, `services/axios.ts`, `services/cities.ts`. Route everything through one config module / env vars.
